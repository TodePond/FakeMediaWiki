...
function emitAgentsMarkdown(methods: MethodSchema[], hooks: HookSchema[]): string {
	...
	lines.push("")
	lines.push(
		"This file lists `FakeWiki` instance methods and Vue composables exported from the `fakewiki` package. Regenerate with `npm run generate` from the repository root or from `packages/fakewiki` (WikiSignals data, API docs, `llms.txt`, and playground schema). To update only the latter three, run: `npx tsx packages/fakewiki/scripts/generate-schema.ts`."
	)
	lines.push("")
	lines.push(
