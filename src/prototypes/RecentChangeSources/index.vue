	...
	<main class="recent-change-sources">
		...
		<section class="stage">
			...
			<div class="prioritize-controls">
				...
				<label class="prioritize-control">
					<span>
						Tone multiplier: {{ formatWeightLabel(toneProbabilityWeight) }}
					</span>
					<input
...
const STORAGE_PREFIX = "prototype.recent-change-sources.v8"
	...
	return [...scoredRows.value]
		.map(revision => {
			...
			const linkTypeWeight = linkTypeWeightStepIds.reduce(
				(total, linkType) =>
					total +
					linkTypeCounts[linkType] *
						linkTypeWeights.value[linkType],
				0
			)
...
function getPageSourceInfoBoxes(
	...
	const effectiveSourceStepIds =
		sourceStepIds?.length ? sourceStepIds : fallbackSourceStepId ? [fallbackSourceStepId] : []
				...
				for (const selectedPage of selectedPages) {
					...
					for (const change of relatedChanges.slice(0, targetCount)) {
						...
						dedupedChanges.set(changeKey, {
							...
							pageSourceLabels: new Set(getQuerySourceLabels(selectedPage.sourceStepIds)),
